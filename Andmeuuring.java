...
//leiab suurima sündide arvuga maakonna vastavalt tulba nr.
    public double maxSynnid(int tulbanr) throws IOException{
        ...
        while(rida!=null){
            String[] m=rida.split(";");
            try{
                double synd=Double.parseDouble(m[tulbanr]);
                if(alustatud){
                    if(synd>maxsynd){maxsynd=synd;}
                } else {
                    maxsynd=synd;
                    alustatud=true;
                }
